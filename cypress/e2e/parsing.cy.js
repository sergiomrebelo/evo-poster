...
for (let lang of Object.keys(inputText)) {
  ...
  describe(`rollup parsing test (lang: ${lang})`, () => {
    ...
    it('automatic division of input text',  () => {
      ...
      // results
      cy.get('#temp-res-sentences').should("be.visible", { timeout: 100000 }).contains(inputText[lang]["automatic"]["sentences"],  { timeout: 100000 });
      cy.get('#temp-res-classification').should("be.visible", { timeout: 100000 }).contains(inputText[lang]["automatic"]["global"],  { timeout: 100000 });
      cy.get('#temp-res-lexicon-lines').should("be.visible",  { timeout: 100000 });
      cy.get('#temp-res-lexicon-lines').first().contains(inputText[lang]["automatic"]["first"],  { timeout: 100000 });
      cy.get('#temp-res-lexicon-global').should("be.visible", { timeout: 100000 }).contains(inputText[lang]["automatic"]["lexicon"],  { timeout: 100000 });
